...
from app.schemas.encounter import IPDAdmissionCreate, IPDAdmissionRead, IPDBedCreate, IPDBedRead, IPDDischarge, IPDSummary
...
def discharge_ipd_admission(
    ...
    admission = IPDService(db).discharge(admission_id, payload.discharge_note, user, context)
    return IPDAdmissionRead.model_validate(admission, from_attributes=True)
