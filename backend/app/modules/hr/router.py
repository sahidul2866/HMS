...
from app.modules.hr.service import HRService, serialize_attendance, serialize_employee, serialize_roster
...
def list_attendance(attendance_date: date | None = None, user=Depends(get_current_user), db: Session = Depends(get_db)) -> list[HRAttendanceRead]:
    return [HRAttendanceRead.model_validate(serialize_attendance(item)) for item in HRService(db).list_attendance(user, attendance_date=attendance_date)]
...
def list_leave_requests(user=Depends(get_current_user), db: Session = Depends(get_db)) -> list[HRLeaveRequestRead]:
    ...
    for item in items:
        ...
        payloads.append(HRLeaveRequestRead.model_validate(data))
    return payloads
...
def request_leave(payload: HRLeaveRequestCreate, user=Depends(get_current_user), db: Session = Depends(get_db)) -> HRLeaveRequestRead:
    ...
    data["employee_name"] = item.employee.full_name if item.employee else None
    data["leave_type_name"] = item.leave_type.name if item.leave_type else None
    return HRLeaveRequestRead.model_validate(data)
